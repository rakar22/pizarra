...
function ModelPanel({ match }) {
  ...
  return <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">MODELO · COMPARATIVA</span><h3>{match.home} — {match.away}</h3></div></div><div className="stats"><Stat label="Poisson Local" value={`${Math.round(analysis.probabilities.home*100)}%`} detail="modelo base"/><Stat label="Dixon-Coles Local" value={`${Math.round(dc.home*100)}%`} detail="corrección baja anotación"/><Stat label="Dixon-Coles Empate" value={`${Math.round(dc.draw*100)}%`} detail="rho = -0.13"/><Stat label="Dixon-Coles +2.5" value={`${Math.round(goals.over*100)}%`} detail="modelo ajustado"/></div><div className="model-comparison"><span>Poisson 1X2</span><b>{Math.round(analysis.probabilities.home*100)}% / {Math.round(analysis.probabilities.draw*100)}% / {Math.round(analysis.probabilities.away*100)}%</b><span>Dixon-Coles 1X2</span><b>{Math.round(dc.home*100)}% / {Math.round(dc.draw*100)}% / {Math.round(dc.away*100)}%</b></div><p className="note">Dixon-Coles está disponible como segunda capa de modelización. La comparación es descriptiva; la selección del modelo debe validarse con histórico out-of-sample.</p></section>;
}
...
function DataPanel({ sources }) { return <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">DATA ENGINE</span><h3>Fuentes registradas</h3></div></div>{sources.map(source => <div className="source-row" key={source.name}><div><strong>{source.name}</strong><span>{source.role}</span></div><b>{source.status}</b></div>)}</section>; }
