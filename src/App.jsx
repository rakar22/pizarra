  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {active === 'Dashboard' && <>
        ...
        <section className="stats"><Stat icon={TrendingUp} label="Partidos con modelo" value={analysed || '—'} detail={withOdds ? `${withOdds} con cuotas disponibles` : 'cuotas aún pendientes'}/><Stat icon={BarChart3} label="Calibración" value="Pendiente" detail="requiere histórico validado"/><Stat icon={Database} label="Datos frescos" value={freshness} detail={`${data.sources.length} fuentes registradas`}/><Stat icon={ShieldCheck} label="Ejecución" value="Desactivada" detail="análisis sin apuestas automáticas"/></section>
      </>}
      ...
      {active === 'Value' && <ValuePanel matches={data.matches} isLive={data.isLive} />}
      {active === 'Modelo' && <ModelPanel match={selected} />}
      {active === 'Scout IA' && <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">SCOUT IA</span><h3>Motor de evidencia</h3></div></div><div className="insight-card"><div className="mini-badge">SIN ALUCINACIONES</div><h4>Interpretación bloqueada si faltan evidencias</h4><p>La capa Scout queda preparada para incorporar lesiones, alineaciones, forma y movimientos de mercado con referencias verificables. No se inventan datos ausentes.</p></div></section>}
      {active === 'Bankroll' && <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">BANKROLL</span><h3>Gestión preparada</h3></div></div><p className="note">El motor de cuarto de Kelly está implementado para análisis, pero la ejecución automática permanece desactivada. No se muestran tamaños de apuesta sin una señal validada y datos de calidad suficiente.</p></section>}
...
function ValuePanel({ matches, isLive }) {
  const withOdds = matches.filter(m => m.odds !== null);
  return <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">VALUE ENGINE</span><h3>Mercado para revisión</h3></div></div>{!withOdds.length ? <div className="empty">No hay cuotas disponibles en los datos actuales. El análisis de value queda pendiente hasta disponer de un mercado normalizado.</div> : <div className="table-wrap"><table><thead><tr><th>PARTIDO</th><th>CUOTA</th><th>MODELO</th><th>ESTADO</th></tr></thead><tbody>{withOdds.map(m => <tr key={m.id}><td><strong>{m.home} — {m.away}</strong><span>{m.league} · {m.source}</span></td><td>{m.odds.toFixed(2)}</td><td>{m.model == null ? 'Pendiente' : `${m.model}%`}</td><td>{isLive ? 'Revisión pendiente' : 'DEMO · no validar'}</td></tr>)}</tbody></table></div>}<p className="note">Value no se marca como oportunidad por una cuota aislada. Requiere probabilidades del modelo, mercado normalizado, calidad de datos y backtesting/calibración histórica.</p></section>;
}

function ModelPanel({ match }) {
  if (!match?.analysis) return <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">MODELO</span><h3>Detalle del partido</h3></div></div><div className="empty">Selecciona un partido con datos suficientes para mostrar el detalle del modelo.</div></section>;
  const { analysis } = match;
  return <section className="panel data-panel"><div className="panel-head"><div><span className="section-kicker">POISSON</span><h3>{match.home} — {match.away}</h3></div><span className="mini-badge">{analysis.method}</span></div><div className="stats"><Stat label="1X2 Local" value={`${Math.round(analysis.probabilities.home * 100)}%`} detail="probabilidad modelada"/><Stat label="Empate" value={`${Math.round(analysis.probabilities.draw * 100)}%`} detail="probabilidad modelada"/><Stat label="1X2 Visitante" value={`${Math.round(analysis.probabilities.away * 100)}%`} detail="probabilidad modelada"/><Stat label="Más de 2.5" value={`${Math.round(analysis.markets.over25 * 100)}%`} detail="probabilidad modelada"/></div><p className="note">Calidad de datos: {Math.round(analysis.dataQuality * 100)}%. Este modelo usa GF/GA de standings; no equivale todavía a un sistema calibrado con histórico out-of-sample.</p></section>;
}
